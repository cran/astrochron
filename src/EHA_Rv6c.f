...
c MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
      ...
      subroutine EHA_Rv6(mpts,npts,inc,nspec,nfreq,dt,imean,itrend,
     &  ifstart,ifend,bufx,tbw,kmany,newpts,tapers,evals,f,amp,phase,
     &  ftest,power,height,ier)

c INPUT:
c mtps = number of time series points
...
c tapers = a vector (mpts x kmany long), contining the dpss tapers (not normalized to RMS=1)
