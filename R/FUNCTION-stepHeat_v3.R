...
stepHeat <- function (dat,unc=1,lambda=5.463e-10,J=NULL,Jsd=NULL,CI=2,cull=T,del=NULL,output=F,idPts=T,size=NULL,unit=1,setAr=95,color="black",genplot=T,verbose=T)
  {

   if(verbose) cat("\n----- GENERATE Ar/Ar AGE SPECTRUM AND CALCULATE WEIGHTED MEAN -----\n")


# force cull=F if del=!NULL
  if(!is.null(del)) cull=F
# force verbose=T if cull=T
  if(cull) verbose=T
# force genplot=T if cull=T
  if(cull) genplot=T
...
# plot weighted mean
    ...
    textsize=1
    if(pfit >= 0.15 && CI == 1) mtext(c(paste("Wt. Mean=",round(mean.w,digits=2),"+/-",round(sigma95,digits=2),"; MSWD=",round(mswd3,digits=2))),side=3,line=1.5,col="red",cex=textsize)
    if(pfit < 0.15 && CI == 1) mtext(c(paste("Wt. Mean=",round(mean.w,digits=2),"+/-",round(mswd95,digits=2),"; MSWD=",round(mswd3,digits=2))),side=3,line=1.5,col="red",cex=textsize)
    if(mswd3 <= 1 && CI == 2) mtext(c(paste("Wt. Mean=",round(mean.w,digits=2),"+/-",round(sigma95,digits=2),"; MSWD=",round(mswd3,digits=2))),side=3,line=1.5,col="red",cex=textsize)
    if(mswd3 > 1 && CI == 2) mtext(c(paste("Wt. Mean=",round(mean.w,digits=2),"+/-",round(mswd95,digits=2),"; MSWD=",round(mswd3,digits=2))),side=3,line=1.5,col="red",cex=textsize)


    if(!cull) mtext(expression(paste("Boxes are 2",sigma," ; Red is wt. mean with 95% CI")),side=3,line=0,col="red")
    if(cull) mtext("Click on dates for exclusion",side=3,line=0)
...
# solve for J uncertainty
  ...
  if(cull) 
    {
      cat("\n * Select dates to remove by clicking on points in the plot on the BOTTOM.\n")
      ...
      if(genplot) 
        {
           ...
           mat <- matrix(c(1,2,3), nrow=3,ncol=1)
           layout(mat, heights=c(1.5,1.5,3.5))  
           plotit(perAr=perAr,x=x,sd=sd,KCa=KCa,Ar40=Ar40,wtMeanOut2[,1],wtMeanOut2[,2],wtMeanOut2[,3],wtMeanOut2[,4],wtMeanOut2[,5],wtMeanOut2[,6],wtMeanOut2[,7],CI=CI,cull=FALSE,tag=tag,size=size,sizeAxis=sizeAxis,unitLabel=unitLabel,setAr=setAr,color=color)
        }
    }

   if(output && !cull) return(wtMeanOut)
   if(output && cull) return(wtMeanOut2)
