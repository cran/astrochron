...
###                  Nov. 27, 2013; January 14, 2014)
###
### Read a time series file. 
###########################################################################

read <- function (d=1,h="auto",srt=T,ave=T,genplot=T)
{
...
###  average duplicates if requested
   cols=length(dat)
   cat(" * Number of variables (columns)=",cols-1,"\n")
   cat("   (excluding first column, which is depth/height/time)\n")
...
### sort to ensure increasing depth/height/time. Will remove NAS values (in depth column)
       ...
       npts <- length(dat[,1])
       cat(" * Number of data points post-sorting=", npts,"\n")
...
### if there are duplicate values, average them (add call to dupmean.f)
      t1<-dat[1:(npts-1),1]
      t2<-dat[2:(npts),1]
      dt=t2-t1
      mindt=min(dt) 
...
### if we have more than two columns, then sort, but only remove if NA in first column. 
###   note that duplicates are not averaged.
   if (cols > 2 && srt)
     {
       cat("\n * Sorting data into increasing depth/height/time order.\n")
       cat("   Will remove empty entries (first column only, empty\n")
       cat("   entries may remain in other columns).\n")
### sort to ensure increasing depth/height/time. Will remove NAS values (in depth column)
       dat <- dat[order(dat[1],na.last=NA,decreasing=F),]
       npts <- length(dat[,1])
       cat("\n * Number of data points post-sorting=", npts,"\n")
    ...
    for (i in 2:cols) 
      {
        ...
        if(class(dat[,i]) == "numeric" || class(dat[,2]) == "integer")
          {
