...
timeOptPlot <- function (dat=NULL,res1=NULL,res2=NULL,simres=NULL,flow=NULL,fhigh=NULL,targetE=NULL,targetP=NULL,xlab="Depth (m)",ylab="Proxy Value",fitR=NULL,verbose=T)
{
...
if(is.null(targetP)) stop("**** ERROR: missing targetP. Terminating now!")

plf=T
if(is.null(flow) || is.null(fhigh)) 
 {
   if(verbose) cat("**** WARNING: flow and/or fhigh not specified, will omit from plot")
   plf=F
 }  
...
if(plf) rect(xleft=flow,ybottom=-0.05,xright=fhigh,ytop=max(fft[, 3])*1.1,col="#FFFF0050",border=NA)            
par(new = TRUE)
plot(fft[, 1], fft[, 3], xlim = c(0, 0.1), type = "l",lwd=1.5, xlab = "", ylab = "", main = "",axes=F)
par(new = TRUE)
plot(fft[, 1], log(fft[, 3]), xlim = c(0, 0.1),ylim=c(-14,-1),type = "l",lwd=1.5, yaxt = "n", col = "gray", xlab = "", ylab = "")
targetTot=append(targetE,targetP)
...
# plot 6: (G)
denPlot=density(simres[,3])
yRange=max(denPlot$y)-min(denPlot$y)
if(max(max(denPlot$x),fitR) > 0.4) xlim2=c(0,1)
if(max(max(denPlot$x),fitR) < 0.4 ) xlim2=c(0,.5)
