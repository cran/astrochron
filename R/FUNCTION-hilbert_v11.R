...
hilbert <- function (dat,padfac=2,demean=T,detrend=F,output=T,outave=F,xmin=dat[1,1],xmax=dat[npts,1],genplot=T,verbose=T)
{
...
### plot some of the results
   par(mfrow=c(3,1))
   plot(fft.out[,1],fft.out[,2], type="l",col="red",ylab="Amplitude", xlab="Frequency", main="Amplitude Spectrum",bty="n")
 }
...
### swapped
      for (j in 2:nyqfreq)
        {
          ht[j]=(Im(ft[j])*-1) + 1i*(Re(ft[j]))
        }
### For frequencies < 0, multiply real and imaginary components by
### -i.  Note that this results in the real and imaginary components being
### swapped
      for (j in negfreq:minusdf)
        {
          ht[j]=Im(ft[j]) + 1i*(Re(ft[j])*-1)
        }
...
### See Kanasewich (1981) for additional information 
         envelope[k] = sqrt ( d[j,2]^2 + Re(ift[j])^2 )
         k=k+1
         }
           
     d <- data.frame(cbind(d[,1],envelope))
...
if(genplot)
  {
     plot(d, type="l",col="red", ylab="Value", xlab="Location", main="Instantaneous Amplitude",bty="n")
