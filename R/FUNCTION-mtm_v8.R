...
mtm <- function (dat,tbw=3,padfac=5,demean=T,detrend=F,siglevel=0.9,xmin=0,xmax=Nyq,ar1=F,output=0,sigID=F,pl=1,genplot=T,verbose=T)
{
...
datTS<-as.ts(dat[,2])
spec <- spec.mtm(datTS,nw=tbw,k=numtap,Ftest=T,nFFT=npad,taper=c("dpss"),centre=c("none"),jackknife=F,returnZeroFreq=F,plot=F)
...
###  We will determine average power directly from measured spectrum
    ...
    for (i in 1:nfreq)  
     {  
        arg = freq[i]/Nyq 
        RawAR[i] = So * (1-(rho_raw^2)) / (  1 - (2*rho_raw*cos(pi*arg)) + (rho_raw^2) )   
     }
    dofAR = (2*numtap)
    chiRawAR <-  (pwrRaw[1:nfreq]/RawAR) * dofAR
...
if(genplot)
 {
   ...
   if(pl == 1)
    {
      plot(freq,log(pwrRaw),type="l", col="black", xlim=c(xmin,xmax), xlab="Frequency",ylab="Log Power",main="MTM Power Estimates",cex.axis=1.1,cex.lab=1.1,lwd=2,bty="n")
      if(ar1) lines(freq[1:nfreq],log(RawAR),xlim=c(xmin,xmax),col="red",lwd=2)
    }
   if(pl == 2)
    {
      plot(freq,pwrRaw,type="l", col="black", xlim=c(xmin,xmax), xlab="Frequency",ylab="Linear Power",main="MTM Power Estimates",cex.axis=1.1,cex.lab=1.1,lwd=2,bty="n")
      if(ar1) lines(freq[1:nfreq],RawAR,xlim=c(xmin,xmax),col="red",lwd=2)
     } 
 }
...
if(genplot)
 {
  if(!ar1) 
   {
     plot(freq,prob*100,type="l",col="red",xlim=c(xmin,xmax),ylim=c(80,100),cex.axis=1.1,cex.lab=1.1,xlab="Frequency",ylab="Confidence Level",main="Confidence Level Estimates",bty="n",lwd=2)
  ...
  if(ar1) 
   { 
     plot(freq[1:nfreq],chiCLRawAR*100,type="l",col="red",xlim=c(xmin,xmax),ylim=c(0,100),cex.axis=1.1,cex.lab=1.1,lwd=2,xlab="Frequency",ylab="Confidence Level",main="AR1 Confidence Level Estimates",bty="n")
     abline(h=90,col="black",lty=3)
     abline(h=95,col="black",lty=3)
     abline(h=99,col="black",lty=3)
     plot(freq,prob*100,type="l",col="red",xlim=c(xmin,xmax),ylim=c(80,100),cex.axis=1.1,cex.lab=1.1,xlab="Frequency",ylab="Confidence Level",main="Harmonic Test Confidence Level Estimates",bty="n",lwd=2)
     abline(h=90,col="black",lty=3)
     abline(h=95,col="black",lty=3)
     abline(h=99,col="black",lty=3)
   } 
...
if (output==1) 
 {
   if(!ar1) 
     { 
       spectrum <- data.frame(cbind(freq[1:nfreq],pwrRaw[1:nfreq],prob[1:nfreq]*100))
   ...
   if(ar1)
     {
       spectrum <- data.frame(cbind(freq[1:nfreq],pwrRaw[1:nfreq],prob[1:nfreq]*100,chiCLRawAR[1:nfreq]*100))
       colnames(spectrum)[1] <- 'Frequency'
       colnames(spectrum)[2] <- 'Power'
       colnames(spectrum)[3] <- 'Harmonic_CL'
