...
###                                   Dec. 25, 2015; May 20-25, 2016; June 7, 2016)
###
...
if(verbose) cat("\n * Data correlation coefficent =",datcor,"\n")
...
#   number of results with abs(surrcor) > abs(datcor)
    ...
    pvalue=pnumgt/nsim
    if(pvalue < (10/nsim) && (10/nsim) <=1 ) cat(" * P-value <", 10/nsim,"\n")
    if(pvalue >= (10/nsim) && (10/nsim) <=1 ) cat(" * P-value =", pvalue,"\n")    
    if((10/nsim) > 1 ) cat(" * P-value = 1 \n")       
# end if nsim >1
}
    ...
    if(!firstDiff)
     {
       if(plSwitch !=3)
        { 
          hist(dat1SR[,2],freq=F, xlab="dat1 data value",main=denTitle1); lines(density(dat1SR[,2], bw="nrd"),col="red",lwd=1.5)
          hist(dat2SR[,2],freq=F, xlab="dat2 data value",main=denTitle2); lines(density(dat2SR[,2], bw="nrd"),col="red",lwd=1.5)
          plot(dat1SR[,2],dat2SR[,2], xlab="dat1 data value", ylab="dat2 data value",main="dat1 vs. dat2")  
        }         
       if(plSwitch ==3)
        { 
          hist(dat1[,1],freq=F, xlab="dat1 data value",main=denTitle1); lines(density(dat1[,1], bw="nrd"),col="red",lwd=1.5)
          hist(dat2[,1],freq=F, xlab="dat2 data value",main=denTitle2); lines(density(dat2[,1], bw="nrd"),col="red",lwd=1.5)
          plot(dat1[,1],dat2[,1], xlab="dat1 data value", ylab="dat2 data value",main="dat1 vs. dat2")  
        }         
     } 
    if(firstDiff) 
     {
       hist(x1SR,freq=F, xlab="dat1 data value",main=denTitle1); lines(density(x1SR, bw="nrd"),col="red",lwd=1.5)
       hist(x2SR,freq=F, xlab="dat2 data value",main=denTitle2); lines(density(x2SR, bw="nrd"),col="red",lwd=1.5)
