...
nfreq = as.integer( round((fmax-fmin)/df) + 1 )
...
if(genplot)
  {
    par(mfrow=c(3,1))
# POWER SPECTRUM
    if(pl == 1) plot(freq,log(pwrRaw),type="l", col="blue", xlab=xlab,ylab="Log Power",main="MTM Power Estimates",cex.axis=1.1,cex.lab=1.1,lwd=2,bty="n")
    if(pl == 2) plot(freq,pwrRaw,type="l", col="blue", xlab=xlab,ylab="Linear Power",main="MTM Power Estimates",cex.axis=1.1,cex.lab=1.1,lwd=2,bty="n")
