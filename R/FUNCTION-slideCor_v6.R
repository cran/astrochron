...
if(genplot)
 {
     ...
     plot(dat1, cex=.5,xlab="",ylab="",main="Optimal Match",bty="n",xlim=c(min(dat1[,1]),max(dat1[,1])))
     start=dat1[1,1]+out[resMax,1]
     end=(dat1[1,1]+out[resMax,1])+(nptsS-1)*dtOut
     par(new=T)
     plot(seq(from=start,to=end,by=dtOut),dat2[,2],type="l",col="red",xlab="",ylab="",main="",bty="n",xaxt="n",yaxt="n",xlim=c(min(dat1[,1]),max(dat1[,1])))
     mtext("Location (spatial or temporal units)",side=1,line=2,cex=0.7)
     mtext("Value",side=2,line=2,cex=0.7)
